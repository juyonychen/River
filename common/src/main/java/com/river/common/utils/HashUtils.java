 ...
 * 其中crc32基于JDK, murmurhash 基于 guava
 *
 *
...
public class HashUtils {
    ...
    private static ThreadLocal<MessageDigest> createThreadLocalMessageDigest(final String digest) {
        return ThreadLocal.withInitial(() -> {
            try {
                ...
                // This exception could never be thrown
                throw new AlgorithmNotExistedException("unexpected exception creating MessageDigest instance for [" + digest + "]", e);
            }
        });
    }
    ...
    public static byte[] md5(String input, @Nullable byte[] salt) {
        return digest(input.getBytes(Charsets.UTF_8), get(MD5_DIGEST), salt, 1);
    }
    ...
    public static class AlgorithmNotExistedException extends RuntimeException {
        private static final long serialVersionUID = -810043694127705311L;

        AlgorithmNotExistedException(String message, Throwable cause) {
            super(message, cause);
        }
    }
}
